...
from .forms import Registration_form, AuthenticateForm, Customerform, ChangePasswordForm, UserProfileForm, AdminProfileForm
from .models import Furniture, Cart, Customer, Order
...
def chair_categories(request):
    cc = Furniture.objects.filter(category = 'CHAIR')
    return render(request, 'core/chair_categories.html', {'cc': cc})

def sofa_categories(request):
    sc = Furniture.objects.filter(category = 'SOFA')
    return render(request, 'core/sofa_categories.html', {'sc': sc})

def table_categories(request):
    tc = Furniture.objects.filter(category = 'TABLE')
    return render(request, 'core/table_categories.html', {'tc': tc})

def single_product(request, id):
    sp = Furniture.objects.get(pk = id)
    return render(request, 'core/single_product.html', {'sp': sp})

def registration(request):
    if not request.user.is_authenticated:
        if request.method == 'POST':
            mf = Registration_form(request.POST)
            if mf.is_valid():
                mf.save()
                messages.success(request,'Registration Successfull !!')
                return redirect('registration')    
        else:
            mf  = Registration_form()
...
def add_to_cart(request,id):
    if request.user.is_authenticated:
        ...
        user = request.user
        Cart(user=user,product=furniture).save()
...
def payment_success(request,selected_address_id):
    ...
    user= request.user
    address_data = Customer.objects.get(pk=selected_address_id)
    cart=Cart.objects.filter(user=request.user)
    for cart in cart:
        Order(user=user,customer=address_data,quantity=cart.quantity,furniture=cart.product).save()
        cart.delete()
...
def buynow(request,id):
    furniture = Furniture.objects.get(pk=id)     # cart_items will fetch product of current user, and show product available in the cart of the current user.
    delhivery_charge =2000
    final_price= delhivery_charge + furniture.discounted_price
    
    address = Customer.objects.filter(user=request.user)

    return render(request, 'core/buynow.html', {'final_price':final_price,'address':address,'furniture':furniture})
...
def buynow_payment(request,id):
    ...
    return render(request, 'core/payment.html', {'final_price':final_price,'address':address,'furniture':furniture,'paypal':paypal_payment})
...
def buynow_payment_success(request, selected_address_id, id):
    ...
    # Fetch the specific furniture instance
    furniture = Furniture.objects.get(pk=id)

    # Pass the actual furniture instance, not the class
    Order.objects.create(user=user, customer=customer_data, furniture=furniture, quantity=1)
